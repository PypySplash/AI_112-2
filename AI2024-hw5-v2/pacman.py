...
# 替换下面的路径为您的系统中 ffmpeg 可执行文件的实际路径
...
def validation(agent, num_evals=5):
    ...
    for i in range(num_evals):
        ...
        while not done:
            ...
            action = agent.act(state, training=False)  # 從代理獲取當前狀態的動作，確保代理處於評估模式
            # get your action feedback from environment
            # next_state, reward, terminated, truncated, info = YOUR_CODE_HERE
            next_state, reward, terminated, truncated, info = eval_env.step(action)  # 使用動作，從環境中獲得下一個狀態
...
def train(agent, env):
    ...
    for _ in tqdm(range(args.max_steps)):
        ...
        if agent.total_steps % args.eval_interval == 0:
            ...
            plt.subplot(1, 2, 1)
            plt.plot(history['Step'], history['AvgScore'], 'r-')
            plt.title('Average Score Over Time')
            plt.xlabel('Step')
            plt.ylabel('Average Score')
            
            plt.subplot(1, 2, 2)
            plt.plot(history['Step'], history['ValueLoss'], 'b-')
...
def evaluate(agent, eval_env, capture_frames=True):
    ...
    if agent is None:
        ...
        # agent.network.load_state_dict(torch.load(args.eval_model_path))
                # 確保模型在 CPU 上
        # agent.network.load_state_dict(torch.load(args.eval_model_path, map_location='cpu'))
        agent.network.load_state_dict(torch.load(args.eval_model_path, map_location='cpu'))

        agent.network.to('cpu')
    
    (state, _), done = eval_env.reset(), False
    # state = state.to('cpu')  # 確保狀態在 CPU 上
    state = torch.from_numpy(state).float().to('cpu')  # 將 numpy array 轉換為 torch tensor 並轉移到 CPU
    ...
    while not done:
        ...
        # state = next_state
        # state = next_state.to('cpu')  # 確保下一狀態在 CPU 上
        # 将numpy数组转换为torch张量，并转移到CPU
        state = torch.from_numpy(next_state).float().to('cpu')
        scores += reward
        done = terminated or truncated
    if capture_frames:
        writer.close()
