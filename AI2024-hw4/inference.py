...
def constructBayesNet(gameState: hunters.GameState):
    ...
    # 設定觀察變數的值域
    observations = list(range(X_RANGE + Y_RANGE - 1 + MAX_NOISE))
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        # 從貝葉斯網絡中獲取考慮證據的所有條件概率表（CPT）
        initialFactors = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        print("Initial Factors with Evidence:", initialFactors)

        # 根據消除順序依次處理每個變量
        for variable in eliminationOrder:
            # 將涉及當前變量的因子合併成一個因子
            currentFactors, combinedFactor = joinFactorsByVariable(initialFactors, variable)
            print(f"Combined Factor for {variable}:", combinedFactor)

            # 如果合併後的因子中非條件變量多於一個，則執行消除操作
            if len(combinedFactor.unconditionedVariables()) > 1:
                reducedFactor = eliminate(combinedFactor, variable)
                # 將消除後的因子加回因子列表
                currentFactors.append(reducedFactor)

            # 更新因子列表
            initialFactors = currentFactors

        # 將所有剩餘因子合併為一個因子
        finalCombinedFactor = joinFactors(initialFactors)
        print("Final Combined Factor:", finalCombinedFactor)

        # 歸一化最終因子以形成正確的條件概率分佈
        normalizedFinalFactor = normalize(finalCombinedFactor)
        return normalizedFinalFactor
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        keys = list(self.keys())
        weights = [self[key] for key in keys]
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        # Check if the ghost is in jail and no noisy distance is observed
        ...
        # If the ghost is not in jail and a noisy distance is observed
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for ghostPosition in self.allPositions:
            obsProb = self.getObservationProb(observation, pacmanPosition, ghostPosition, self.getJailPosition())
            newBeliefs[ghostPosition] = self.beliefs[ghostPosition] * obsProb
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        repeats = self.numParticles // len(self.legalPositions) + 1  # Ensure enough particles to distribute
        self.particles = (self.legalPositions * repeats)[:self.numParticles]
    ...
    def getBeliefDistribution(self):
        ...
        # Create a belief distribution from the list of particles
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        pacmanPosition = gameState.getPacmanPosition()
        newWeights = DiscreteDistribution()

        # Update weights for each particle based on the observation and Pacman's position
        for particle in self.particles:
            noisyDistance = observation
            trueDistance = manhattanDistance(pacmanPosition, particle)
            prob = self.getObservationProb(noisyDistance, pacmanPosition, particle, self.getJailPosition())
            newWeights[particle] += prob

        # Check if all particles have zero weight and reinitialize if necessary
        if newWeights.total() == 0:
            self.initializeUniformly(gameState)
            for particle in self.particles:
                newWeights[particle] = 1  # Reset weights uniformly

        # Resample particles based on updated weights
        newParticles = []
        for _ in range(self.numParticles):
            newParticles.append(newWeights.sample())
    ...
    def elapseTime(self, gameState):
        ...
        # For each particle, predict its next state based on the current state and the game state
        for oldParticle in self.particles:
            # Get the transition model distribution for the current particle
            newPosDist = self.getPositionDistribution(gameState, oldParticle)

            # Sample a new position from the transition model
